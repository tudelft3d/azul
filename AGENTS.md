...
- Object type filter hides objects in the sidebar and in the 3D view: `DataManager::objectTypeFilter` (empty set = all types) with a memoized tri-state `AzulObject::matchesTypeFilter` ('Y'/'N'/'U'); an object matches if its own type is selected or any descendant matches, so ancestors stay visible for context. Sidebar rows are skipped in the accessors; 3D geometry is hidden by folding `matchesTypeFilter` into `updateVisibleStates()` (no buffer regeneration needed, picking culls too since the picking shader uses the same states). `availableTypesWithCounts()` feeds the UI menus and skips structural rows (`"File"`, `"LoD"`, `"lod"+digits`). The filter is not persisted and resets on file load.
